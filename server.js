...
app.delete('/api/admin/users/:id', authMiddleware, adminMiddleware, (req, res) => {
  ...
  const tx = db.transaction(() => {
    // Gruppen aufräumen: wenn User Owner war, Nachfolger bestimmen oder Gruppe löschen
    const ownedGroups = db.prepare(`SELECT id FROM groups WHERE owner_user_id = ?`).all(userId);
    for (const group of ownedGroups) {
      ensureGroupOwnerStillMember(group.id, userId);
    }
    db.prepare(`DELETE FROM group_members WHERE user_id = ?`).run(userId);
    db.prepare(`UPDATE characters SET group_id = NULL WHERE user_id = ?`).run(userId);
